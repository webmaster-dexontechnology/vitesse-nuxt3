...
const imageUrl = '/dexon/pipeline-services/auth_cover_lxo6fz.webp'
const data = reactive({
  username: '',
  password: '',
  loading: false,
...
<template>
  <div class="pt-custom-xl h-screen w-screen container lg:max-w-full lg:p-0">
    <div class="flex flex-col lg:flex-row">
      <div class="relative w-full lg:h-screen lg:w-[55%] xl:w-[60%]">
        <div class="inset-0 z-10 lg:absolute lg:p-16">
          <div class="mb-custom-lg mx-auto max-w-xl lg:mb-20 lg:max-w-none">
            <svg
              class="h-7 fill-primary lg:h-[40px] lg:fill-white" xmlns="http://www.w3.org/2000/svg"
            ...
            >
              <defs>
                ...
                >
                  <stop offset="0" class="text-[#ffffff] dark:text-[#ec0677]" stop-color="currentColor" />
                  <stop offset="1" class="text-[#ffffff] dark:text-[#ec1b34]" stop-color="currentColor" />
                </linearGradient>
              </defs>
              <path
              ...
              />
              <g>
                <path d="M156.14,59.73h-2.95v8.03h-1.6v-8.03h-2.95v-1.32h7.51v1.32Z" class="fill-[#808285] dark:fill-[#ffffff]" />
                <path
                  d="M165.42,59.73v2.67h3.87v1.32h-3.87v2.71h5.21v1.32h-6.81v-9.35h6.81v1.32h-5.21Z"
                  class="fill-[#808285] dark:fill-[#ffffff]"
                />
                <path
                  ...
                  class="fill-[#808285] dark:fill-[#ffffff]"
                />
                <path d="M200.72,58.41v9.35h-1.6v-4.03h-4.1v4.03h-1.6v-9.35h1.6v4h4.1v-4h1.6Z" class="fill-[#808285] dark:fill-[#ffffff]" />
                <path
                ...
                <path
                  ...
                  class="fill-[#808285] dark:fill-[#ffffff]"
                />
                <path d="M247.15,66.44v1.32h-6.55v-9.35h1.6v8.03h4.94Z" class="fill-[#808285] dark:fill-[#ffffff]" />
                <path
          ...
          <h1 class="hidden max-w-2xl text-3xl font-bold tracking-[-0.01rem] uppercase text-white lg:block lg:text-[38px] xl:text-[42px] lg:leading-[1.3] xl:leading-[1.3]">
            we are <span class="text-secondary-red-100">leading technology</span> providing non-destructive testing and in-line inspection
          </h1>
        </div>
        <nuxt-img loading="lazy" sizes="lg:1280px, sm:900px, xs:600px" alt="Login Page" class="hidden h-full w-full object-cover object-center opacity-95 lg:flex" :src="myCld.image(imageUrl).delivery(quality(80)).toURL()" />
        <div class="absolute inset-y-0 left-0 z-[-1] hidden h-full w-full bg-primary lg:flex" />
      </div>
      <div class="w-full lg:w-[45%] xl:w-[40%]">
        <div class="h-full flex flex-col items-center justify-center">
          <div class="grid max-w-xl w-full lg:px-16 xl:px-18">
            <h2 class="sm:mb-custom-md mb-custom-sm text-xl font-bold capitalize md:text-3xl sm:text-2xl">
              {{ $t('form.auth.login_title') }}
            </h2>
            <FormInput v-model="data.username" class="mb-custom-md w-full" type="text" :value="null" :placeholder="$t('form.auth.placeholder.username')" :label="$t('form.auth.fill.username')" />
            <FormInput v-model="data.password" class="mb-custom-lg w-full" type="password" :value="null" :placeholder="$t('form.auth.placeholder.password')" :label="$t('form.auth.fill.password')" />
            <div class="w-full">
              <UIButton
                class="md:text-md bg-red-gradient items-center whitespace-nowrap rounded-md px-12 py-3 text-sm font-semibold text-white ring-secondary-red-100/25 transition duration-200 ease-out hover:ring-4" liquid :disabled="isButtonDisabled" @click="handleLogin"
              >
                <span>{{ $t('form.auth.login') }}</span>
              </UIButton>
            </div>
          </div>
        </div>
